...
class XhtmlPrepper
{
    ...
    private attributeFacts($: CheerioAPI, factMap: Map<string, SingleFact>, docSlug: string)
    {
        const startPerformance = performance.now();

        const factElements = Array.from($(`[contextRef]`));

        factElements.forEach((factElem) => {
            $(factElem).attr("selected-fact", 'false');
            $(factElem).attr("hover-fact", 'false');
            $(factElem).attr("continued-fact", 'false');
            $(factElem).closest('table').length ? $(factElem).attr("inside-table", 'true') : $(factElem).attr("inside-table", 'false');

            if (!$(factElem).prop('tagName').toLowerCase().endsWith("continuation") && $(factElem).attr("continuedat")) {
                $(factElem).attr("continued-main-fact", 'true');
            }

            if ($(factElem).attr('contextref') && $(factElem).attr('id')) {
                $(factElem).attr('ix', $(factElem).attr('id'));
                $(factElem).attr('id', this.updateMap($(factElem).attr('ix') as string, $(factElem), docSlug));
            }

            if ($(factElem).attr('contextref') && !$(factElem).attr('id')) {
                // for the facts in the html that have no ids...
                const getByNameAndContextRef = (contextRef: string | undefined, concept: string | undefined) => {
                    for (const [factId, factObj] of factMap.entries()) {
                        if (factObj.contextRef === contextRef && factObj.name === concept) {
                            return factId;
                        }
                    }
                };

                const mapKey = getByNameAndContextRef($(factElem).attr('contextref'), $(factElem).attr('name'));
                if (mapKey) {
                    $(factElem).attr('ix', mapKey);
                    $(factElem).attr('id', mapKey);
                    this.updateMap($(factElem)?.attr('ix') as string, $(factElem), docSlug);
                } else {
                    const log: Logger<ILogObj> = new Logger();
                    log.error(`Fact [name] && [contextRef] could not be located in the Map Object.`);
                }
            }
            $(factElem).wrap(`<span></span>`);
        });
