                ...
                # type-specific validations
                    ...
                    for name in names:
                        for f in sevFacts(sev, name, requiredContext=not axisKey, whereKey="where", sevCovered=subTypes != {"n/a"}):
                            if inferredDecimals(f) > maxDecimals:
                                sevMessage(sev, subType=submissionType, modelObject=f, efmSection=efmSection, tag=ftName(name), label=ftLabel(name), value=f.xValue, decimals=inferredDecimals(f), maxDecimals=maxDecimals, ftContext=ftContext(axisKey,f))
                                # avoid writing to store-db-object since this is an invalid value
                                f.xValid = INVALID
