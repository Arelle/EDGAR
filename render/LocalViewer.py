...
EDGAR_VIEWER_ROOT = os.path.join(os.path.dirname(os.path.dirname(__file__)), "view")
...
webRootDirPattern = re.compile(r"^include$")
ixvRootDirPattern = re.compile(r"^i[x-z]viewer")
...
class _LocalViewer(LocalViewer):
    ...
    def getLocalFile(self, file, relpath, request):
        ...
        # self.cntlr.addToLog(f"_report={_report} _file={_file}", messageCode="localViewer:trace", level=logging.DEBUG)
